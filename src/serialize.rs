use std::iter::repeat;
use cookie_factory::{GenError, set_be_u8, set_be_u32};

use ssh::{SshPacket, SshPacketKeyExchange, SshPacketDhReply, SshPacketDebug,
          SshPacketDisconnect};


fn gen_string<'a, 'b>(x: (&'a mut [u8], usize), s: &'b [u8]) -> Result<(&'a mut [u8], usize), GenError> {
    do_gen!(
        x,
        gen_be_u32!(s.len()) >>
        gen_slice!(s)
...
fn gen_packet_key_exchange<'a, 'b>(x: (&'a mut [u8], usize), p: &'b SshPacketKeyExchange) -> Result<(&'a mut [u8], usize), GenError> {
    do_gen!(
        x,
...
fn gen_packet_dh_reply<'a, 'b>(x: (&'a mut [u8], usize), p: &'b SshPacketDhReply) -> Result<(&'a mut [u8], usize), GenError> {
    do_gen!(
        x,
        gen_string(p.pubkey_and_cert) >>
        gen_string(p.f) >>
        gen_string(p.signature)
    )
}


fn gen_packet_disconnect<'a, 'b>(x: (&'a mut [u8], usize), p: &'b SshPacketDisconnect) -> Result<(&'a mut [u8], usize), GenError> {
    do_gen!(
        x,
        gen_be_u32!(p.reason_code) >>
        gen_string(p.description) >>
        gen_string(p.lang)
    )
}


fn gen_packet_debug<'a, 'b>(x: (&'a mut [u8], usize), p: &'b SshPacketDebug) -> Result<(&'a mut [u8], usize), GenError> {
    do_gen!(
        x,
...
fn gen_packet_payload<'a, 'b>(x: (&'a mut [u8], usize), p: &'b SshPacket) -> Result<(&'a mut [u8], usize), GenError> {
    match *p {
...
pub fn gen_ssh_packet<'a, 'b>(x: (&'a mut [u8], usize), p: &'b SshPacket) -> Result<(&'a mut [u8], usize), GenError> {
    do_gen!(
        x,
        len:      gen_skip!(4) >>
        padlen:   gen_skip!(1) >>
